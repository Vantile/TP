...
import es.ucm.fdi.tp.practica6.bgame.model.GameClient;
import es.ucm.fdi.tp.practica6.bgame.model.GameServer;
...
public class Main {
	...
	/**
	 * Numero de obstaculos por defecto.
	 */
	final private static int defObstacles = 0;
	...
	/**
	 * Numero de obstaculos que va a tener el juego. Solo disponible en Ataxx.
	 */
	private static int obstacles = defObstacles;
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructAppModeOption()); // -am o --app-mode
		cmdLineOptions.addOption(constructServerPortOption()); //-sp o --server-port
		cmdLineOptions.addOption(constructServerHostOption()); // -sh o --server-host
	...
	private static Option constructAppModeOption(){
		return new Option("am", "app-mode", true,
				"Indica el modo en el que ejecutar el juego (normal, client o server). Por defecto, normal.");
	}
	
	private static void parseAppModeOption(CommandLine line){
		String mode = line.getOptionValue("am", "NORMAL");
		if(mode.equals("NORMAL") || mode.equals("CLIENT") || mode.equals("SERVER"))
	...
	private static Option constructServerPortOption(){
		return new Option("sp", "server-port", true,
				"Indica el puerto al que debe conectarse el cliente o donde debe iniciar el servidor.");
	}
	
	private static void parseServerPortOption(CommandLine line){
		serverPort = Integer.parseInt(line.getOptionValue("sp", "2000"));
	}
	
	private static Option constructServerHostOption(){
		return new Option("sh", "server-host", true,
				"Indica el nombre de la maquina sobre la que se est· ejecutando el servidor.");
	}
	
	private static void parseServerHostOption(CommandLine line){
		serverHost = line.getOptionValue("sh", "localhost");
	}
	...
	/**
	 * Construye la opcion de los obstaculos (-o o --obstacles).
	 * @return Objeto {@link Option} de esta opcion.
	 */
	private static Option constructObstaclesOption()
	{
		String optionInfo = "Number of obstacles you want to put on the board "
				+ "(pieces that don't belong to any player and can't be moved).";
		optionInfo += "By default, " + defObstacles + ".";
	...
	/**
	 * Parsea la opcion obstaculos y asigna el valor de obstacles.
