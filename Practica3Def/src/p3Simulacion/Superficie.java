...
/**
 * Superficie del mundo del juego.
 * Las celulas se mueven por la superficie.
 */
public class Superficie {
	...
	public boolean posicionDisponibleSimple(int f, int c){
		...
		while((fila <= f + 1) && !disponible)
		{
			...
			while((columna <= c + 1) && !disponible)
			{
				 ...
				 */
				if(fila < 0 || columna < 0 || fila >= this.nf || columna >= this.nc)
					disponible = false;
				else
	...
	public boolean posicionDisponibleCompleja(int f, int c) {
		 ...
		 */
		while(i < this.nf && !disponible)
		{
			j = 0;
			while(j < this.nc && !disponible)
			{
	...
	public void guardar(FileWriter fw) throws IOException
	{
		try{
			for(int i = 0; i < this.nf; i++)
			{
				for(int j = 0; j < this.nc; j++)
				{
					if(this.posicionOcupada(i, j))
					{
						fw.write(i + " " + j + " ");
						this.superficie[i][j].guardar(fw);
